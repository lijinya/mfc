...
{
	//创建图像列表
	m_iNormal.Create(IDB_NORMAL,16,1,RGB(0,0,0));
	m_wndTree.SetImageList(&m_iNormal,TVSIL_NORMAL);
	HTREEITEM hRoot=m_wndTree.InsertItem("达内集团",0,1);	//选中0 未选中则为1
	HTREEITEM hCBD=m_wndTree.InsertItem("CBD中心",2,3,hRoot);
	HTREEITEM hVIP=m_wndTree.InsertItem("VIP中心",2,3,hRoot);
	HTREEITEM hTAM=m_wndTree.InsertItem("天安门中心",2,3,hRoot);
///	HTREEITEM hCBD=m_wndTree.InsertItem("CBD中心",2,3,hRoot);
	m_wndTree.InsertItem("csd1201",2,3,hTAM);
	m_wndTree.InsertItem("csd1202",2,3,hTAM);
	m_wndTree.InsertItem("csd1203",2,3,hTAM);
